...
import { getAnnouncementsByClassId } from "../../dataaccess/announcement";
...
export default async function handleViewAnnouncementsByClassId(
  req: Request,
  res: Response,
  ...
  try {
    ...
    const response = await getAnnouncementsByClassId(
      user.centreId,
      parseInt(id)
    );
